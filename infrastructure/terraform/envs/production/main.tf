...
module "vm" {
  ...
  # operator to be on the network / a full-tunnel VPN). Flip to true and set
  ...
  # cloud-init: format/mount the attached Cinder volume at /var/lib/docker
  # BEFORE Docker is installed by Ansible.
  user_data = <<-EOF
    #cloud-config
    runcmd:
      - |
